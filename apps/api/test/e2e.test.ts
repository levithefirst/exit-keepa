...
const app = createApp();
let token: string;
beforeEach(async () => { callContractFunction.mockReset(); getDirectExecutionStatus.mockReset(); token = await createTestSession(fakeDb, "0xAbC0000000000000000000000000000000AbC1"); });
...
describe("end-to-end: create strategy -> condition true -> simulate -> execute -> recorded", () => {
  it("walks the full authorized-execution flow and records a real, validated tx hash", async () => {
    ...
    callContractFunction.mockResolvedValueOnce({ success: true, status: "simulated", wouldRevert: false }); const simRes = await request(app).post(`/api/exit-strategies/${strategyId}/executions/${executionId}/simulate`).set(authHeader(token)); expect(simRes.status).toBe(200); expect(simRes.body.status).toBe("simulated"); expect(callContractFunction.mock.calls[0][0].simulate).toBe(true);
    const REAL_HASH = "0x" + "a".repeat(64); callContractFunction.mockResolvedValueOnce({ result: REAL_HASH }); const broadcastRes = await request(app).post(`/api/exit-strategies/${strategyId}/executions/${executionId}/broadcast`).set(authHeader(token)); expect(broadcastRes.status).toBe(200); expect(broadcastRes.body.status).toBe("succeeded"); expect(broadcastRes.body.txHash).toBe(REAL_HASH); expect(callContractFunction.mock.calls[1][0].simulate).toBe(false);
    const listRes = await request(app).get(`/api/exit-strategies/${strategyId}/executions`).set(authHeader(token)); expect(listRes.status).toBe(200); expect(listRes.body[0].txHash).toBe(REAL_HASH); const secondBroadcastRes = await request(app).post(`/api/exit-strategies/${strategyId}/executions/${executionId}/broadcast`).set(authHeader(token)); expect(secondBroadcastRes.status).toBe(200); expect(secondBroadcastRes.body.txHash).toBe(REAL_HASH); expect(callContractFunction).toHaveBeenCalledTimes(2);
  });
