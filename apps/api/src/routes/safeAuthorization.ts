...
function serializeSafeTx(tx: ReturnType<typeof buildSafeTransaction>) {
  return { to: tx.to, value: tx.value.toString(), data: tx.data, operation: tx.operation, safeTxGas: tx.safeTxGas.toString(), baseGas: tx.baseGas.toString(), gasPrice: tx.gasPrice.toString(), gasToken: tx.gasToken, refundReceiver: tx.refundReceiver, nonce: tx.nonce.toString() };
}
...
function normalizeSafeSignature(signature: string): `0x${string}` {
  ...
  if (v === 0 || v === 1) return `${signature.slice(0, -2)}${(v + 27).toString(16).padStart(2, "0")}` as `0x${string}`;
...
async function predictedCode(address: `0x${string}`) {
  ...
  if (!response.ok) throw new HttpError(502, "Could not verify the module installation. Try again.");
  const body = await response.json() as { result?: string };
  return body.result ?? "0x";
}

async function nextPlan(id: string, safeAddress: `0x${string}`, owner: `0x${string}`) {
  const row = await getSafeRow(id, owner);
  if (row.isSandbox) return { inspection: null, modifier: null, plan: [] as const };
  ...
  if (!modifier) {
    ...
    const code = await predictedCode(predicted);
    if (code !== "0x") {
  ...
  if (!(await verifyEnabledModule(safeAddress, modifier))) {
    const tx = buildSafeTransaction({ to: safeAddress, data: encodeEnableModule(modifier), nonce: inspection.nonce });
  ...
  const calls = buildRoleConfigurationCalls(safeAddress, keeper);
  if (!permission.memberAssigned) return safeConfigStep(safeAddress, modifier, calls[0], inspection.nonce);
  if (!permission.targetScoped) return safeConfigStep(safeAddress, modifier, calls[1], inspection.nonce);
  if (!permission.functionScoped) return safeConfigStep(safeAddress, modifier, calls[2], inspection.nonce);
  ...
  return { inspection, modifier, plan: [] as const };
}
...
function responseFor(row: any, result: Awaited<ReturnType<typeof nextPlan>>) {
  const first = result.plan[0];
  const state = result.plan.length === 0 ? "protected" : first?.kind === "deploy_proxy" ? "needs_module" : "needs_permission";
  return { state, status: state === "protected" ? "protected" : "needs_authorization", safeAddress: row.safeAddress, modifierAddress: result.modifier, plan: result.plan, canonicalRoleKey: canonicalRoleKey() };
}
...
safeAuthorizationRouter.post("/safe-accounts/:id/authorization/execute-calldata", async (req, res) => {
  ...
  const signature = normalizeSafeSignature(String(req.body?.signature ?? ""));
  ...
  const tx = buildSafeTransaction({ to: step.safeTx!.to as `0x${string}`, data: step.safeTx!.data as `0x${string}`, nonce: BigInt(step.safeTx!.nonce) });
  ...
  const execData = encodeExecTransaction(buildSafeTransaction({ to: tx.to, data: tx.data, nonce: current.nonce }), signature);
