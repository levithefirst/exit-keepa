...
export async function detectZodiacModifier(safeAddress: string): Promise<{ enabledModules: string[]; modifier: string | null; incompatibleRoles: boolean }> { const enabledModules = await inspectEnabledModules(safeAddress as `0x${string}`); let modifier: string | null = null; let incompatibleRoles = false; for (const moduleAddress of enabledModules) { const classification = await classifyRolesModule(moduleAddress as `0x${string}`, safeAddress as `0x${string}`); if (classification === "compatible") modifier = moduleAddress; if (classification === "incompatible_roles") incompatibleRoles = true; } return { enabledModules, modifier, incompatibleRoles }; }
