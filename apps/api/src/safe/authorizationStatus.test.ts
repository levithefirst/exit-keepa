...
function stubChain(options: { modules?: string[]; modifierAvatar?: string; modifierTarget?: string; modifierOwner?: string; implementation?: string; failModulesRead?: boolean; exactPermission?: boolean }) {
  vi.stubGlobal("fetch", vi.fn(async (_url: unknown, init?: RequestInit) => {
    const body = JSON.parse((init?.body as string) ?? "{}"); const method = body.method; const params = body.params ?? []; const call = params[0] ?? {}; const to = method === "eth_getCode" ? String(params[0] ?? "").toLowerCase() : String(call.to ?? "").toLowerCase(); const data = String(call.data ?? "");
