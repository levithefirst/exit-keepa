...
function encodeModules(modules: string[]): string { return `0x${word("0x40")}${word("0x1")}${word(BigInt(modules.length).toString(16))}${modules.map(word).join("")}`; }
...
function permissionPointerCode(): string {
  const assetHash = keccak256(encodeAbiParameters([{ type: "address" }], [AAVE_V3_BASE.usdc]));
  const recipientHash = keccak256(encodeAbiParameters([{ type: "address" }], [SAFE]));
  return `0x00${"00a5003000200030"}${assetHash.slice(2)}${recipientHash.slice(2)}`;
}
function stubChain(options: { modules?: string[]; modifierAvatar?: string; modifierTarget?: string; modifierOwner?: string; implementation?: string; failModulesRead?: boolean; exactPermission?: boolean }) {
  vi.stubGlobal("fetch", vi.fn(async (_url: unknown, init?: RequestInit) => {
    ...
    if (method === "eth_getCode") {
      const result = to === MODIFIER.toLowerCase() ? rolesProxyCode(options.implementation) : to === POINTER.toLowerCase() ? permissionPointerCode() : "0x6000";
      return new Response(JSON.stringify({ jsonrpc: "2.0", id: 1, result }), { status: 200 });
    }
    if (method === "eth_getStorageAt") {
      if (options.exactPermission) {
        const calls = (stubChain as unknown as { storageCalls?: number }).storageCalls ?? 0;
        (stubChain as unknown as { storageCalls?: number }).storageCalls = calls + 1;
        const result = calls === 0 ? `0x${word("0x1")}` : calls === 1 ? `0x${word("0x2")}` : `0x${(4n << 240n | BigInt(POINTER)).toString(16).padStart(64, "0")}`;
        return new Response(JSON.stringify({ jsonrpc: "2.0", id: 1, result }), { status: 200 });
      }
      const result = `0x${word("0x1")}`;
      return new Response(JSON.stringify({ jsonrpc: "2.0", id: 1, result }), { status: 200 });
    }
...
describe("authorization", () => {
  ...
  it("reports needs_permission when the exact on-chain permission is incomplete", async () => {
  ...
  it("reports protected only after the exact Roles state and negative probes succeed", async () => {
    stubChain({ modules: [MODIFIER], exactPermission: true });
    const status = await readAuthorizationStatus({ ...realSafe, rolesKey: canonicalRoleKey() }, ACTION);
  ...
  it("fails closed when module state cannot be read", async () => {
    ...
    const status = await readAuthorizationStatus(realSafe, ACTION);
    expect(status.state).toBe("undetermined");
    expect(status.undetermined).toMatch(/Could not verify/);
  });
});
