...
async function rpcCall(to: string, data: Hex, from?: string): Promise<string> { const result = await rpc("eth_call", [{ to, data, ...(from ? { from } : {}) }, "latest"]); if (typeof result !== "string" || !result) throw new HttpError(409, "Safe verification returned no result"); return result; }
...
function decodeAddressArray(hex: string): string[] { const body = hex.slice(2); const offset = Number(BigInt(`0x${body.slice(0, 64)}`)) * 2; const length = Number(BigInt(`0x${body.slice(offset, offset + 64)}`)); const result: string[] = []; for (let i = 0; i < length; i++) result.push(`0x${body.slice(offset + 64 + i * 64 + 24, offset + 64 + i * 64 + 64)}`); return result; }
...
export async function inspectSafeForAuthorization(safeAddress: `0x${string}`, connectedOwner: `0x${string}`) {
  ...
  return { owners, threshold: Number(BigInt(thresholdRaw)), nonce: BigInt(nonceRaw), version: decodeString(versionRaw), masterCopy, isOwner: owners.some((o) => o.toLowerCase() === connectedOwner.toLowerCase()), isSafe: code !== "0x" && masterCopy.toLowerCase() === SAFE_V1_4_1_SINGLETON.toLowerCase() };
}
...
export async function verifyNegativeRoleProbes(modifierAddress: `0x${string}`, safeAddress: `0x${string}`, memberAddress: `0x${string}`): Promise<boolean> {
  ...
  for (const call of probes) { try { await rpcCall(modifierAddress, encodeFunctionData({ abi: ROLE_EXEC_ABI, functionName: "execTransactionWithRole", args: [call.to, call.value, call.data, call.operation, roleKey, true] }), memberAddress); return false; } catch { /* expected */ } }
  return true;
}
